...
if (!$therapist_id) {
  die("Therapist not selected.");
}


$stmt = $pdo->prepare("SELECT full_name, profile_picture FROM users u
...
<body>
  ...
  <img src="../uploads/<?= $therapist['profile_picture'] ?>" alt="Therapist Photo">
  <h2><?= htmlspecialchars($therapist['full_name']) ?></h2>
  <p>Therapist</p>
</div>

    <h1>Book a Session</h1>

    <form action="../php/book_request.php" method="POST">
  ...
  <input type="hidden" name="therapist_id" value="<?= htmlspecialchars($therapist_id) ?>">
      ...
      <div class="form-group">
        <label for="message">Optional Message to Therapist</label>
        <textarea id="message" name="message" placeholder="You may add a note or question..."></textarea>
      </div>
